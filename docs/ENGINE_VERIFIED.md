...
- User provided collapsed-node copy-back with six outputs; consecutive pin rows are modeled at 32px (two 16px cells), independent of horizontal knot offset (16px). `PIN_ROW_H=32`; one-step spacing applies generically to pins by row index. `tools/gen-collapsed-knot-test.mjs` supersedes the rejected 3×3 probe with a 4-in/5-out Composite. It places 9 port knots at ±32px, 7 between-port knots another ±16px outward, and models first pin center at NodePosY+56px from the supplied reference. STRICT OK; awaiting UE visual confirmation.
