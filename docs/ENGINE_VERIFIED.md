...
- User provided collapsed-node copy-back with six outputs; consecutive pin rows are modeled at 32px (two 16px cells), independent of horizontal knot offset (16px). `PIN_ROW_H=32`; one-step spacing applies generically to pins by row index. `tools/gen-collapsed-knot-test.mjs` produces an unconnected 3-in/3-out Composite geometry probe with nine staggered knots; STRICT OK, awaiting UE visual confirmation.
