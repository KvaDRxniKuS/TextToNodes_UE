...
- Width correction after UE visual feedback: Composite was incorrectly fixed at 256px. Width now derives from visible pin-label body geometry and snaps to the nearest 16px grid. The calibrated 3×3 reference stays at 256px; the 1×3 shape estimates to 240px, so its first right port Knot is x=272 (32px beyond the estimated right edge), then x=288/304 for subsequent levels. STRICT/test validated; re-check visually in UE.
