...
- Width correction after latest UE copy-back: a fixed/rounded-down 240px estimate was too narrow. The supplied live reference has Composite `NodePosX=-11440` and right port Knot `K2Node_Knot_9402 NodePosX=-11152`; with the measured 32px port gap, Composite width is 256px. The 1×3 fixture therefore uses right port column x=NodePosX+288, then x+304/x+320 for two recursive levels. Knot `K2Node_Knot_17` is also at x=-11232 in the reference; do not infer width from that knot without its functional role. STRICT/test validated.
