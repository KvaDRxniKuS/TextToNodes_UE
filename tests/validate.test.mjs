...
// Раскладка: layoutRow без наложений (O-фидбек: Δ320 перекрывал трейды)
{
  ...
  // Exec rows align by pin center, accounting for one-line vs two-line headers.
  {
    ...
    ok(pinCenterY(one, outElse) - pinCenterY(one, outThen) === 48, 'layoutRows: Branch.then и Branch.else — разные вертикальные ряды');
  }
...
// R26 VERIFIED + R30 pre: декор (--decorate): exec-knot'ы на переносе рядов
{
  ...
  const execAlign = mk(['--decorate', '--wrap', '4', 'event A', 'fn Branch', 'fn ClearAllMappings', 'fn FlushPlayerInput', 'link 1.then 2.execute', 'link 2.then 3.execute', 'link 2.else 4.execute']);
  ...
  ok(flushA.pos.y - clearA.pos.y === 48, `Exec alignment: Branch true/false branches separated by two 16px grid steps (${clearA.pos.y}/${flushA.pos.y})`);
